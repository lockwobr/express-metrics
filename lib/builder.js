...
function initServer(port, isCluster, statsdConfig, influxdbConfig) {
  var statsd = null;
  ...
  server = new Server(port, statsd, statsdRoutes, influxdbConfig);
...
module.exports.init = function init(options) {
  ...
  if (!options.cluster && options.port) {
    initServer(options.port, false, options.statsd, options.influxdb);
  }
